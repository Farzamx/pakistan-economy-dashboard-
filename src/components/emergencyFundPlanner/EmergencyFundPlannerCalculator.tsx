...
import EmergencyFundPlannerForm, { type EmergencyFundFormValues } from "@/components/emergencyFundPlanner/EmergencyFundPlannerForm";
import EmergencyFundPlannerResults from "@/components/emergencyFundPlanner/EmergencyFundPlannerResults";
import { useEconomicProfile, setEconomicProfile, getEffectiveMonthlyBudget } from "@/lib/decisionSupportLab/economicProfile";
...
const DEFAULT_RETURN_PCT = 6; // conservative — emergency funds prioritize liquidity/safety over growth
const DEFAULT_VOLATILITY_PCT = 2;

interface Props {
  breakdown: CpiCategoryBreakdown | null;
}

export default function EmergencyFundPlannerCalculator({ breakdown }: Props) {
  ...
  const goal = getPrimaryGoalOfType(profile.goals, "emergency-fund");

  function handleCreate(values: EmergencyFundFormValues) {
    const monthlyExpenses = getEffectiveMonthlyBudget(profile);
    const targetDate = new Date();
    targetDate.setFullYear(targetDate.getFullYear() + 2); // a 2-year default horizon to build the fund — editable via monthly contribution
    const nextGoals = addGoal(profile.goals, {
      name: `${values.monthsOfCoverage}-Month Emergency Fund`,
      goalType: "emergency-fund",
      targetAmountToday: values.monthsOfCoverage * monthlyExpenses,
  ...
  const scenarios: ScenarioCardData[] = useMemo(() => {
    ...
    return (
      [
        { label: "Pessimistic" as const, delta: -3 },
        { label: "Base Case" as const, delta: 0 },
        { label: "Optimistic" as const, delta: 3 },
      ] as const
    ).map(({ label, delta }) => {
  ...
  const confidence = useMemo(
    () =>
      calculateConfidenceScore({
        ...
        manualEstimateCount: 1, // expected return is always a manual assumption for this tool
  ...
  const recommendations = useMemo(
    () =>
      progress && goal
        ...
        : [],
    [profile, progress, goal, effectiveGoal],
  );
  ...
  function buildReport(): ReportDefinition {
    if (!progress || !goal || !monteCarloResult) {
      return { toolName: "Emergency Fund Planner", generatedAt: new Date().toISOString().slice(0, 10), sourceNote: "Source: Pakistan Economic Intelligence Center", sections: [] };
    }
    return {
  ...
  if (!goal) {
    return (
      <div className="flex flex-col gap-6">
        <EmergencyFundPlannerForm defaultCurrentSavings={profile.currentSavings} defaultMonthlyExpenses={getEffectiveMonthlyBudget(profile)} onCreate={handleCreate} />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <ConfidenceBadge result={confidence} toolId="emergency-fund-planner" />
      <DataFreshnessBadge sourceName="Pakistan Bureau of Statistics" lastUpdated={breakdown?.observationDate ?? ""} dataFrequency="Monthly" />

      {progress && <EmergencyFundPlannerResults progress={progress} monthsOfCoverage={Math.round(goal.targetAmountToday / Math.max(1, getEffectiveMonthlyBudget(profile)))} />}
      ...
      <ExplainTheMath
        ...
        variables={[
          { symbol: "Inflated Target", description: "Months of coverage × your monthly spending, inflated to the target date" },
          { symbol: "r", description: "Your expected monthly return (annual ÷ 12)" },
          { symbol: "n", description: "Months remaining to your target date" },
        ]}
        methodology="Your dedicated savings and monthly contributions are projected forward using the same compounding math as every Time Value of Money tool in this Lab, then compared against your target inflated by your chosen inflation assumption — the classic funding-gap calculation used by professional financial planners."
        ...
        limitations={["The probability simulation assumes annual returns are normally distributed around your expected return — real markets can behave differently, especially over short horizons."]}
        provenance={provenance}
      />

      <EducationalPanel
        whatDoesThisMean="This shows whether your emergency fund, growing at your expected savings rate, will be large enough — after inflation — to cover the number of months of expenses you chose."
        whyDifferent="An emergency fund's real target rises every year prices do, since it needs to cover the same months of spending at future prices, not today's."
        howCalculated="Your current dedicated savings and monthly contributions are compounded forward at your expected return, then compared against your target (months × monthly spending) inflated to your target date."
        sources={["Pakistan Bureau of Statistics — National CPI Index (Base 2015-16 = 100)", "Your Economic Profile (monthly spending, savings)"]}
      />

      <DecisionSupportPanel
        whatHappened={progress ? `Your Emergency Fund is projected to be ${progress.isOnTrack ? "on track" : `${progress.fundingGapPct.toFixed(0)}% short of`} its inflation-adjusted target.` : "Set up your goal to see a projection."}
        ...
        whatToUnderstand="An emergency fund prioritizes safety and liquidity over growth — a low, stable expected return and volatility assumption is usually more realistic here than for a long-horizon investment goal."
        relatedTools={[{ title: "Wealth Accumulation Planner", href: "/decision-support-lab/wealth-accumulation-planner" }, { title: "Personal Economic Health Score", href: "/decision-support-lab/health-score" }]}
