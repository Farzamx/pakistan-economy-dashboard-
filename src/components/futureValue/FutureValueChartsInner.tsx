...
export default function FutureValueChartsInner({ presentValueAmount, annualReturnPct, years, frequency }: Props) {
  ...
  const series = buildGrowthSeries(presentValueAmount, annualReturnPct, years, compoundingsPerYear);
  ...
  return (
    <div ref={containerRef}>
      <h3 className="mb-2 text-sm font-semibold text-white light:text-slate-900">{t("futureValue.timelineTitle")}</h3>
      {shouldRender && (
        <ResponsiveContainer width="100%" height={240}>
          <AreaChart data={data} margin={{ top: 4, right: 16, left: 8, bottom: 4 }}>
            ...
            <Area type="monotone" dataKey="value" stroke="#34d399" strokeWidth={2} fill="url(#fv-fill)" isAnimationActive={!prefersReducedMotion} animationDuration={700} />
          </AreaChart>
        </ResponsiveContainer>
      )}
    </div>
