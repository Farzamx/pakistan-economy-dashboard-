    ...
    <div className="flex flex-col gap-6">
      <div className="grid gap-4 sm:grid-cols-3">
        <div className="glass-card flex flex-col gap-1 rounded-xl p-4">
          <span className="text-xs text-[var(--text-muted)]">{t("budget.fyValue")}</span>
          <span className="text-2xl font-semibold text-[var(--text-primary)]">
            {value !== null ? `Rs ${value.toFixed(value >= 1000 ? 0 : 1)}bn` : "—"}
          </span>
        </div>
        <div className="glass-card flex flex-col gap-1 rounded-xl p-4">
          <span className="text-xs text-[var(--text-muted)]">{t("budget.shareOfTotal")}</span>
          <span className="text-2xl font-semibold text-[var(--text-primary)]">
            {shareOfBudget !== null ? formatPct(shareOfBudget) : "—"}
          </span>
        </div>
        <div className="glass-card flex flex-col gap-1 rounded-xl p-4">
          <span className="text-xs text-[var(--text-muted)]">{t("budget.shareOfGdp")}</span>
          <span className="text-2xl font-semibold text-[var(--text-primary)]">
      ...
      {disclaimer && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.06] p-4 text-sm text-amber-200">
          <strong>{t("budget.note")}</strong> {disclaimer}
        </div>
      )}

      <div className="glass-card flex flex-col gap-4 rounded-2xl p-5">
        <div className="flex items-center justify-between">
          <h3 className="text-sm font-semibold text-[var(--text-primary)]">FY2010-11 — FY2026-27 {t("budget.trendTitle")}</h3>
