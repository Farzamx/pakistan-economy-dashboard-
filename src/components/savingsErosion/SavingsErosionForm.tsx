...
export default function SavingsErosionForm({ savingsAmount, onSavingsAmountChange, inflationPct, onInflationPctChange, years, onYearsChange }: Props) {
  const { t } = useLanguage();
  ...
  return (
    <div className="glass-card grid grid-cols-1 gap-4 rounded-xl p-4 sm:grid-cols-3 sm:p-5">
      ...
      <div>
        <label htmlFor="se-inflation" className="text-label text-white/40 light:text-slate-400">
          {t("savingsErosion.inflationLabel")}
        </label>
        <div className="mt-1.5 flex items-center gap-2 rounded-lg border border-[var(--border-subtle)] px-3 py-2.5">
          <input
            id="se-inflation"
            ...
            inputMode="decimal"
            step={0.1}
            value={inflationPct === 0 ? "" : inflationPct}
            placeholder="0"
            onChange={(e) => {
              const parsed = parseFloat(e.target.value);
              onInflationPctChange(isNaN(parsed) ? 0 : parsed);
            }}
            className="text-mono-num w-full bg-transparent text-lg font-semibold tabular-nums text-white outline-none light:text-slate-900"
          />
